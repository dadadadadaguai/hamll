...
//    private final IItemService itemService;
    ...
    public void addItem2Cart(CartFormDTO cartFormDTO) {
        ...
        // 2.判断是否已经存在
        if (checkItemExists(cartFormDTO.getItemId(), 1L)) {
            // 2.1.存在，则更新数量
    ...
    private void handleCartItems(List<CartVO> vos) {
        Set<Long> itemsIds = vos.stream().map(CartVO::getItemId).collect(Collectors.toSet());
        //发起远程调用
//        ServiceInstance instance = ServiceSearch.getDiscoveryClient(discoveryClient, "item-service");
//        ResponseEntity<List<ItemDTO>> response = restTemplate.exchange(
//                instance.getUri()+"itms?ids={ids}",
//                HttpMethod.GET,
//                null,
//                new ParameterizedTypeReference<List<ItemDTO>>() {
//                },
//                Map.of("ids", CollUtils.join(itemsIds, ","))
//        );
